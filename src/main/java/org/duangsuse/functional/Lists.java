...
public final class Lists {
  static <T> LinkedList<T> linked() { return new LinkedList<T>(); }
  ...
  static void impossible() throws RuntimeException
    { throw new RuntimeException("Inconsistent program state reached"); }
  ...
  static <T> List<T>[] partitionWhile(final List<T> xs, Predicate<T> p, Class<T> reifiedT) {
    final List<T> ls = linked(), rs = linked();
    boolean musttake = false; // state1

    // 没有真泛型真是跟吃了 *　一样的难受；标准库没有 Pair 真是...
    for (T x : xs) {
      boolean drop = p.test(x); // extermely inefficent
      if (!musttake && drop) {
        ls.add(x);
      } // state0 <- drop = drop
      else if (!musttake && !drop) {
        musttake = true;
        rs.add(x);
      } // state0 <- take => state1
      else if (musttake)
        rs.add(x); // state1 <- drop/take = take
      else if (!drop)
        rs.add(x); // state* <- take = take
      else
        impossible();
    }
  ...
  @SuppressWarnings("unchecked")
  static <T> List<T> dropWhile(final List<T> xs, Predicate<T> p) {
    return partitionWhile(xs, p, (Class<T>) xs.get(0).getClass())[1];
  }

  @SuppressWarnings("unchecked")
  static <T> List<T> takeWhile(final List<T> xs, Predicate<T> p) {
  ...
  static <A, B> List<B> flatMap(final List<A> xs, Function<A, Collection<? extends B>> f) {
    List<B> ys = linked();
    for (A x : xs)
      ys.addAll((Collection<? extends B>) f.apply(x));
    return ys;
  }
}
